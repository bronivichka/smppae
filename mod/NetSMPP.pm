# NetSMPP.pm  -  SMPP over TCP, pure perl implementation
...
sub encode_optional_params {
    ...
    while (@_) {  # N.B. by using array instead of hash we can control order of items
        ...
        if ($param_by_name{$opt_param} and $opt_param eq 'receipted_message_id') {
            $data .= pack 'nnZ*', $param_by_name{$opt_param}, length($val)+1, $val;
        } elsif ($param_by_name{$opt_param} and $opt_param eq 'ussd_service_op') {
#            $data .= pack 'nnC', 1281, , length($val), $val;
            $data .= pack 'nnC', 1281, 1, $val;
        } elsif ($param_by_name{$opt_param} and $opt_param eq 'source_port') {
            $data .= pack 'nnn', $param_by_name{$opt_param}, 2, $val;
        } elsif ($param_by_name{$opt_param}) {
            $data .= pack 'nna*', $param_by_name{$opt_param}, length($val), $val;
        } elsif ($opt_param == 8210) {  # tariff charge
            $data .= pack 'nnn', $opt_param, , length($val), $val;
...
# linas function for simply do new() call
sub new {
    my $proto = shift;
    ...
    for my $a (keys %{&Default}) {
        ${*$self}{$a} = exists $arg{$a} ? $arg{$a} : Default->{$a};
    }

    $self->set_version(${*$self}{smpp_version});
...
# linas function for build smpp packet from encoded data
# encode_pdu($me, $op, &encode, @params);
sub encode_pdu {
    my $me = shift;
    my $op = shift;
    my $data = shift;
...
# linas function to build smpp resp pdu from encoded data
sub encode_resp_pdu {
    my $me = shift;
    my $op = shift;
    my $data = shift;
    ...
    for (my $i=0; $i <= $#_; $i+=2) {
	    next if !defined $_[$i];
	    if ($_[$i] eq 'seq')    { $seq = splice @_,$i,2,undef,undef; }
	    elsif ($_[$i] eq 'status') { $status = splice @_,$i,2,undef,undef; }
    }
    croak "seq must be supplied" if !defined $seq;
    $status = 0 if !defined $status;
    ...
    return ($seq, $header.$data);
}

# linas functions (not sending packet, only build) 
sub bind_transceiver_pdu { $_[0]->encode_pdu(CMD_bind_transceiver, &encode_bind, @_) }
sub enquire_link_pdu { my $me = $_[0]; $me->encode_pdu(${*$me}{smpp_version}==0x40?CMD_enquire_link_v4:CMD_enquire_link, '', @_) }
sub enquire_link_resp_pdu { my $me = $_[0]; $me->encode_resp_pdu(${*$me}{smpp_version}==0x40?CMD_enquire_link_resp_v4:CMD_enquire_link_resp, '', @_) }
sub unbind_resp_pdu { my $me = $_[0]; $me->encode_resp_pdu(CMD_unbind_resp, '', @_) }
sub unbind_pdu { my $me = $_[0]; $me->encode_pdu(CMD_unbind, '', @_) }
sub submit_sm_pdu { my $me = $_[0]; $me->encode_pdu(CMD_submit_sm, ${*$me}{smpp_version}==0x40 ? &encode_submit_v4 : &encode_submit_v34, @_) }
sub deliver_sm_resp_pdu { my $me = $_[0]; $me->encode_resp_pdu(CMD_deliver_sm_resp, ${*$me}{smpp_version} == 0x40 ? '' : &encode_submit_resp_v34, @_) }

# linas functions for getting smpp error codes
sub get_smpperror {
    my $me = shift;
    return ${*$me}{smpperror};
}
...
sub explain_status {
    ...
    return sprintf("%s (%s=0x%08X)",
		   NetSMPP::status_code->{${*$me}{status}}->{msg},
		   NetSMPP::status_code->{${*$me}{status}}->{code},
		   ${*$me}{status});
}
...
sub read_hard {
	...
	if (!defined($n)) {
	    ...
	    push @{$me->{smpperror}}, ["read_hard I/O error: $!", 1];
	    return undef;
	}
	if (!$n) {
	    warn "premature eof reading from socket";
	    push @{$me->{smpperror}}, ["read_hard premature eof", 2];
	    return undef;
	}
    }
    #warn "read complete";
...
# linas function for decode incoming data
# Тут надо обязательно (!!!!) написать обработку inbound_buffer в цикле, пока ВСЕ пакеты из него не будут вынуты
sub decode_pdu {
    my $me = shift;
    my $data = shift;
    my ($len, $pdu, $header);
    my $header_len = ${*$me}{head_len};

    my $pdus = [];

    # Добавляем только что полученное в конец нашего буфера
    ${*$me}{inbound_buffer} .= $data;

    my $done = 0;
    
    while (!$done) {
        # Сначала обрабатываем заголовок smpp pdu
        # У нас пустой inbound_pdu, значит, мы еще не получали полного заголовка
        if (!defined(${*$me}{inbound_pdu})) {
            # Не получили еще полный заголовок
            if (length(${*$me}{inbound_buffer}) < $header_len) {
                $done = 1;
                next;
            } else {
                $header = substr(${*$me}{inbound_buffer}, 0, $header_len);
                ${*$me}{inbound_buffer} = substr(${*$me}{inbound_buffer}, $header_len);
            }
            
            $pdu = { cmd => 0, status => 0, seq => 0, data => '', };
            ($len, $pdu->{cmd}, $pdu->{status}, $pdu->{seq}, $pdu->{reserved}) = unpack ${*$me}{head_templ}, $header;
            if ($len < $header_len) {
                # Это некая ошибка, выгребем раненый пакет и продолжим
                # Как это вообще может быть???
                # inbound_pdu мы еще не успели изменить, из буфера эту часть уже вынули
                push @{${*$me}{smpperror}}, ["decode_pdu too short header length $len < $header_len: cmd=$pdu->{cmd}, status=$pdu->{status}, seq=$pdu->{seq}", 3];
                next;
            }
           
            # Это длина данных в пакете
            $len -= $header_len;
        # У нас уже в прошлый раз сформировался какой-то заголовок
        # Но в прошлый раз данных не хватило на сам пакет
        } else {
            # Заголовок
            $pdu = ${*$me}{inbound_pdu};
            $len = ${*$me}{inbound_pdu}->{__data_length__};
            delete(${*$me}{inbound_pdu}->{__data_length__});
        }
        #warn "read Header:\n".hexdump($header, "\t") if $trace;
        
        # Обрабатываем теперь тело (данные) smpp pdu
        # В буфере не хватает данных для целого пакета
        if (length(${*$me}{inbound_buffer}) < $len) {
            ${*$me}{inbound_pdu} = $pdu;
            ${*$me}{inbound_pdu}->{__data_length__} = $len;
            $done = 1;
            next;
        # В буфере есть хотя бы один целый пакет
        } else {
            ${*$me}{inbound_pdu} = undef;
            $pdu->{data} = substr(${*$me}{inbound_buffer}, 0, $len);
            ${*$me}{inbound_buffer} = substr(${*$me}{inbound_buffer}, $len);
        }
        ...
        if (defined pdu_tab->{$pdu->{cmd}}) {
            ...
            push @$pdus, bless $pdu => 'NetSMPP::PDU';
        }
    }
...
sub read_pdu {
	...
	warn "Too short length $len < ${*$me}{head_len}, cmd=$pdu->{cmd}, status=$pdu->{status}, seq=$pdu->{seq}";
        push @{$me->{smpperror}}, ["read_pdu: Too short length $len < ${*$me}{head_len}, cmd=$pdu->{cmd}, status=$pdu->{status}, seq=$pdu->{seq}", 3];
	return undef;
    }
    warn "read Header:\n".hexdump($header, "\t") if $trace;
    ...
    $me->read_hard($len, \$pdu->{data}, 0) or do {
        push @{$me->{smpperror}}, ["read_pdu: invalid length cmd=$pdu->{cmd},status=$pdu->{status}, seq=$pdu->{seq}", 3];
        return undef;
    };
    warn "read Body:\n".hexdump($pdu->{data}, "\t") if $trace;
...
NetSMPP - pure Perl implementation of SMPP 3.4 over TCP

=head1 SYNOPSIS

  use NetSMPP;
  $smpp = NetSMPP->new_transciever($host, port=>$port,
...
In synchronous mode request PDU methods return a NetSMPP::PDU object
...
like this:
    ...
    ...
    $resp_pdu = $smpp->wait_pdu(NetSMPP::CMD_query_sm_resp, $seq)
       or die;
...
corresponding response and return NetSMPP::PDU object representing
...
innards of NetSMPP (see C<Default> table from line 217
...
C<param_tab> in the NetSMPP source code, around line 345, for list of
...
In general the NetSMPP module does not enforce SMPP
...
Similarily, if the remote end returns incorrect PDUs and NetSMPP is
able to parse them (usually because length fields match), then NetSMPP
...
Reads a PDU from stream and analyzes it into NetSMPP::PDU
...
Typical client:
  ...
  use NetSMPP;
  $smpp = NetSMPP->new_transciever('smsc.foo.net', Port=>2552) or die;
...
NetSMPP was originally written for version 3.4 of SMPP protocol. I
...
NetSMPP is copyright (c) 2001 by Sampo Kellomaki, All rights reserved.
Portions copyright (c) 2001 by Symlabs, All rights reserved.
You may use and distribute NetSMPP under same terms as perl itself.
